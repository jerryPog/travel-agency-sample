...
import { CheckCircle2, Sparkles, Star, ArrowRight, ShieldCheck, Calculator, Users, Calendar } from 'lucide-react';
...
export function PricingPlansSection() {
  ...
  const plans = [
    {
      ...
      features: [
        ...
        'Skip-the-line Eiffel Tower & Louvre timed entries',
        'Self-guided walking routes with local tips',
        'Direct WhatsApp support before your trip',
      ],
    ...
    },
    {
      ...
      isPopular: true,
      features: [
        '5–7 day full day-by-day bespoke itinerary',
        'Private Seine River Sunset Cruise with Champagne',
        'Handpicked boutique hotels, cafés & hidden bistros',
        'Dedicated Paris travel consultant',
    ...
    },
    {
      ...
      isPopular: false,
      features: [
        '100% custom itinerary tailored to your exact pace',
        'Private Mercedes Chauffeur & Private Licensed Guide',
        'Day trips included (Versailles, Champagne & Giverny)',
        'Personal luxury travel concierge available anytime',
        'VIP Airport Meet & Greet + Priority transfers',
      ],
  ...
  // Dynamic Quote Calculation (€120 base/day + €80/person/day)
  ...
  return (
    <section id="packages" className="w-full max-w-[1440px] mx-auto px-6 md:px-12 py-16 text-white font-['DM_Sans',sans-serif]">
      ...
      <div className="text-center max-w-3xl mx-auto mb-10">
        <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-white/10 border border-white/20 text-xs font-semibold text-amber-300 mb-3 shadow-lg">
          ...
          <span>Curated Pricing & Plans</span>
        </div>
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold font-['Plus_Jakarta_Sans',sans-serif] tracking-tight text-white mb-3">
          Transparent Packages Tailored to Your Journey
        </h2>
        <p className="text-xs sm:text-sm text-white/70">
          No hidden fees or generic tour groups. Pick a pre-crafted tier or calculate a custom quote below.
        </p>
        ...
        {filteredPlans.map((plan) => (
          <div
            ...
            className={`relative rounded-3xl p-6 sm:p-8 flex flex-col justify-between transition-all duration-300 glass-card-hover ${
              plan.isPopular
                ? 'bg-gradient-to-b from-amber-500/20 via-white/10 to-white/5 border-2 border-amber-400 shadow-2xl scale-[1.02]'
      ...
      <div className="bg-white/5 backdrop-blur-2xl border border-white/15 rounded-3xl p-6 sm:p-8 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          ...
          <div className="md:col-span-4 bg-white/10 border border-white/20 rounded-2xl p-5 text-center space-y-2">
            <div className="text-xs text-white/70 font-medium">Estimated Package Total</div>
            <div className="text-3xl font-extrabold text-amber-300 font-['Plus_Jakarta_Sans',sans-serif]">
              €{estimatedTotal.toLocaleString()}
            </div>
            <div className="text-[11px] text-white/50">Includes hotels, skip-line passes & guide</div>
            <button
              onClick={handleChoosePlan}
