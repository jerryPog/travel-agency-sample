...
export function BannerHeroContent({ onCtaClick }: BannerHeroContentProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center px-4 max-w-3xl mx-auto my-auto py-6 sm:py-8 z-10 relative">
      {/* Top Floating Experience Badge */}
      <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-xs font-semibold text-amber-300 mb-3 sm:mb-4 shadow-lg animate-float">
        <Sparkles className="w-3.5 h-3.5" />
        <span>{t('heroBadge')}</span>
      </div>

      {/* Main Headline */}
      <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-[46px] font-extrabold text-white tracking-tight leading-[1.14] max-w-2xl mb-3 sm:mb-4 font-['Plus_Jakarta_Sans',sans-serif]">
        {t('heroHeadline')}
      </h1>

      {/* Subtitle */}
      <p className="text-xs sm:text-sm md:text-base text-white/85 font-normal max-w-lg mb-5 sm:mb-6 leading-relaxed">
        {t('heroSubtitle')}
      </p>
